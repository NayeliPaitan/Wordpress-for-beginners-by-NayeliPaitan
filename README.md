# 📚 Curso Profesional de WordPress – Desde Cero

Material académico desarrollado y dictado por Nayeli P.  
Curso realizado en colaboración con una ONG y la Universidad de Ciencias y Humanidades (UCH).

---

## 🎯 Objetivo del Curso

Capacitar a estudiantes sin conocimientos técnicos en la creación, gestión y publicación de páginas web utilizando WordPress.

---

## 🧩 Contenido del Curso

✔ Instalación y configuración de WordPress  
✔ Gestión de páginas y contenidos  
✔ Creación y administración de blogs  
✔ Gestión de plugins  
✔ Administración de usuarios  
✔ Creación de piezas gráficas con Canva  
✔ Optimización y exportación para la web  

---

## 🛠 Tecnologías Utilizadas

- WordPress
- XAMPP
- MySQL / MariaDB
- Elementor
- Canva
- Gestión de Hosting local

---

## 👩‍🏫 Competencias Demostradas

- Diseño instruccional técnico
- Estructuración de contenido educativo
- Dominio práctico de WordPress
- Gestión de herramientas web
- Experiencia docente con instituciones

---

## 📜 Licencia

Este material fue creado por Nayeli P.  
Uso educativo permitido. No redistribución comercial sin autorización.

---

## 🌐 Sobre la Autora

Ingeniera en Sistemas  
Desarrolladora Web  
Creadora de contenido tech  
